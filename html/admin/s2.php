...
$leftstr=$left['cnt'];
$oid=$_GET['id'];
$pics = db::rows("select a.id, offer_id, b.name from UploadPictures a join apps b on a.offer_id=b.id where type='DoneApp' and reviewed=$status and a.offer_id=$oid order by b.id limit 60");
foreach($pics as $p){
 $id=$p['id'];
