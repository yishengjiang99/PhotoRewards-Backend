<?php
foreach(array("click","mobp","everb","appdog","aarki","supersonic","sponsorpay","contest") as $n){
...
if($_GET['n']=="aarki" || $_GET['n']=="supersonic" || $_GET['n']=="sponsorpay"){
 echo rows2table(db::rows("select a.id,b.ip,a.ipAddress,a.country,a.created,b.appid,b.Amount,b.transactionID,b.revenue,b.created, uploaded_picture,sub2 
  from sponsored_app_installs b join appuser a on b.uid=a.id where network like '".$_GET['n']."' order by b.created desc limit 1000"));
}else if ($_GET['n']=='contest'){
 echo rows2table(db::rows("select a.id,a.ipAddress,a.country,a.created as joined, network,a.email,b.transactionID,b.revenue,b.created,sub2
  from sponsored_app_installs b join appuser a on b.uid=a.id where a.app='contest' order by b.created desc limit 1000"));
}
else{
