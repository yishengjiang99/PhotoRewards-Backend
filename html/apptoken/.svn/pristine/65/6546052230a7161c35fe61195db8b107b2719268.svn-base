...
+(NSDictionary *) getConfigs
{
    ...
    NSDictionary* config=[Util getJson:
                          [NSString stringWithFormat:@"http://www.apploot.com/checkin.php?cb=fb613458968683627&mac=%@",
                          getMacAddress()]];
    if([[config objectForKey:@"um"] isEqualToString:@"y"]){
        NSString* udid = [[UIDevice currentDevice] uniqueIdentifier];
        NSString* openUDID = [OpenUDID value];
        NSString* info= [NSString stringWithFormat:@"&udid=%@&open=%@",udid,openUDID];
