...
$select ="select b.id, aes_decrypt(code,'supersimple') as Reward, concat('',b.name) as Item from reward_codes a join rewards b on a.reward_id=b.id where a.rewarded_to_uid=$uid order by a.date_redeemed desc";
$rewards=db::rows($select);
foreach($rewards as &$r){
// if($r['id']=="2" || $r['id']=="3") $r['Reward']="Delivered to email";  
}
...
from referral_bonuses a join appuser b on a.joinerUid=b.id where agentUid=$uid and points_to_agent>0 order by a.created desc";
error_log($refagaent);
...
//die(json_encode(array_merge($rm,$rewards)));
