...
if($country!="VN" && $locale!="" && $locale!="vi_VN" && $user['banned']!=1 && $ltv>100){
...
if($uid==2902 || $uid==29184){
  $hash=md5("what1sdns?".$idfa.$mac);
  $url="http://m.appdog.com/m/aff.jsp?exchange=300&currency=Points&source=9135311512939222220&idfa=$idfa&fbid=$uid&&hash=$hash";
...
if($reviewer==0 && $locale=='en_US' && $start==0 && $user['banned']!=1){
...
$minpage=40;
if(!$dogon) $minpage=10;
if($locale!="" && $country!="VN" && $locale!="vi_VN" && $user['banned']!=1 && $user['fbfriends']>4 && $start==$minpage){
