...
if($uid>0 && !isset($_COOKIE['tutshown'])){
 $showtut=1;
 setcookie("tutshown",1,time()+36000);
}
$pic=null;
if(isset($r['pid'])){
 $pid=stripslashes($r['pid']);
 $pic=db::row("select a.id as pid,a.points_earned, b.cash_value as up, a.uid, b.click_url, b.id as oid, b.thumbnail,b.storeID,b.name,b.affiliate_network from UploadPictures a join offers b on a.offer_id=b.storeID 
where a.id='$pid' and active=1");
}
if(!$pic && isset($r['appid'])){
   $appid=intval($r['appid']);
   $pic=db::row("select a.id as pid,a.points_earned, b.cash_value as up, a.uid, b.click_url, b.id as oid, b.thumbnail,b.storeID,b.name,b.id as offer_id,b.affiliate_network from UploadPictures a join offers b on $
where b.storeID=$appid and b.active>0 and b.affiliate_network='everbadge'  order by a.uid=$uid desc limit 1");
}
if(!$pic && isset($r['oid'])){
   $oid=intval($r['oid']);
   $pic=db::row("select a.id as pid,a.points_earned, b.cash_value as up, a.uid, b.click_url, b.id as oid, b.thumbnail,b.storeID,b.name,b.id as offer_id,b.affiliate_network from UploadPictures a join offers b on a.offer_id=b.storeID
where b.id=$oid and b.active>0 order by a.uid=$uid desc limit 1");
}
if(!$pic){
	...
	$e_msg="You already sharing this picture";
     ...
     if($user['tracking']<=20 || $uid==2902){
      ...
      if($uid!=$uploaderId){
        ...
        $newlink=bitlyLink("http://www.json999.com/pr/p22.php?uid=$uploaderId&pid=$pid");
...
$picture="http://json999.com/pr/uploads/$pid.jpeg";
$cb="https://www.json999.com/pr/p22.php?uid=$uid&pid=$pid&h=".md5($uid.$pid."asdfadss")."&o=liked";
...
$publish="http://www.facebook.com/dialog/feed?app_id=146678772188121&link=".urlencode($url)."&name=".urlencode($title)."&caption=".urlencode($caption)."&redirect_uri=".urlencode($cb);
...
<?php }?>
<br><b><?php echo "<a href='$clickurl'>$title</a>" ?></b>
<br><img height=200px align=middle src='http://json999.com/pr/uploads/<?php echo $pid?>.jpeg'>
<?php if($uid>0){
 echo "<br><b>Share this app with your friends!<br>Earn $extra points when they download!";
...
<table width=100%><tr>
<td><a class='btn' href='/pr/p22.php?uid=<?php echo $uid ?>&cmd=prev'><- Prev </a></td>
<td colspan=2><a class='btn-large' href='<?php echo $publish ?>'>Share This App</a></td>
<td><a class='btn' href='/pr/p22.php?uid=<?php echo $uid ?>&cmd=next'>Next -> </a></td>
</tr></table>
</div>
</div>
<script>
...
<?php if($showtut==1) {?>
alert("Share App Screenshots and earn points instantly!\n\nShare the 'direct link' to your friends and earn MORE points when they download the App!");
<?php } ?>

<?php if($e_msg!="") {?>
var msg="<?php echo $e_msg ?>";
alert(msg);
window.location="http://www.json999.com/pr/p22.php?uid=<?= $uid ?>";
<?php } ?>
</script>
</html>
