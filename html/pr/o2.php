...
foreach($offers as $offer){
 $oid=$offer['offer_id'];
 if($offer['geo']!=''){
  $geo=explode(",",$offer['geo']);
  if(!in_array($country,$geo)){
	continue;
//    error_log("skipping $oid for country $country");
  }else{
//   error_log("showing $oid for country $country");
  }
 }
 if($oid==97 && $country!='GB') continue;
