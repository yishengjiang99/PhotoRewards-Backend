...
main() {
	...
	// open 
	if((fd = open("/dev/CDD2/CDD2_b", O_RDWR | O_TRUNC)) == -1) {
		fprintf(stderr,"ERR:on open():%s\n",strerror(errno));
		exit(0);
	}
	fprintf(stderr,"open(/dev/CDD2/CDD2_b)=%d\n",fd);
	...
	//  to test if the O_TRUNC flag works.  
    //  it is not specified to CDD2_c dev, similar to CDD2_b
    //  note that the "Hello Universe" from Makefile is *not* 
    //  printed on  /var/log/messages for CDD2_b.
    //  and, it gets printed for CDD2_c

	if((fd = open("/dev/CDD2/CDD2_c", O_RDWR | O_APPEND)) == -1) {
		fprintf(stderr,"ERR:on open():%s\n",strerror(errno));
		exit(0);
	}
	fprintf(stderr,"open(/dev/CDD2/CDD2_c)=%d\n",fd);
