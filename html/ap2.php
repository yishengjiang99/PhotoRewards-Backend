...
if($appjson['results'] && $appjson['results'][0]){
 ...
 $desc=addslashes($apparr['description']);
 $name=addslashes($name);
 db::exec("insert ignore into apps set id=$appid, Name='$name',IconURL='$imgurl',RedirectURL='$url',screenshot='$screenshots',description='$desc' on duplicate key update description='$desc'");
 if($app=db::row("select * from apps where id=$appid")){
 	die(json_encode($app));
  }
}
