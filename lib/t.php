...
function getSSAFeed($user, $limit, $start=0){
  $ssa=array();
  ...
  foreach($rayoffers as $offer){
   ...
   if(in_array($storeId,$installs) || in_array($storeId,$rclicks){
              continue;
   }
...
function curlSSAFeed($user,$country,$device,$osVersion, $limit, $start=0){
  ...
  $rclicks=db::row("select group_concat(offer_id) as clicked from contest_clicks where uid=$uid and created>date_sub(now(), interval 1 day)");
  $rclicks=explode(",",$rclicks['clicked']);
  $installs=db::row("select group_contat(offer_id) as installed from sponsored_app_installs where uid=$uid");
  $installs=explode(",",$rclicks['installed']);
  if($fbid!=0){
  ...
  $page=($start+10)/10; 
  $url="http://www.supersonicads.com/delivery/mobilePanel.php?applicationUserId=$uid&page=$page&pageSize=20&applicationKey=2d5dc8b9&deviceOs=ios&deviceIds[IFA]=$idfa%20&deviceModel=$device";
  ...
  $url.="&publisherSubId=".getRealIP();//."_contest";
  error_log($url);
